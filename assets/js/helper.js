var sliceHtml = {};
sliceHtml.spTags = ["img", "br", "hr"]; /*不需要成对出现的标记*/
sliceHtml.contain = function (arr, it) {
...
sliceHtml.slice = function (article, worldNum) {
  var result = [];
  ...
  for (var x = 0, len = startTags.length; x < len; x++) {
    if (!sliceHtml.contain(sliceHtml.spTags, startTags[x])) {
